...
export default function TargetsPage() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const res = await fetch(apiUrl(`/api/targets?userId=${user.id}`));
  ...
  async function createTarget(e: React.FormEvent) {
    ...
    await fetch(apiUrl("/api/targets"), {
      method: "POST",
  ...
  async function markQuizResult(id: number, correct: boolean) {
    await fetch(apiUrl(`/api/targets/${id}`), {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function toggleStatus(t: Target) {
    await fetch(apiUrl(`/api/targets/${t.id}`), {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function remove(id: number) {
    await fetch(apiUrl(`/api/targets/${id}`), { method: "DELETE" });
    load();
  }
